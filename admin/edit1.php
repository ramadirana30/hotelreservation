...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		...
		<div class="row">
			<div class="col-lg-12">
			<?php
			$query = mysqli_query($koneksi, "SELECT * FROM tbl_booking");
			while($d = mysqli_fetch_array($query)){
				?>
                <h1 class="page-header">Edit Data</h1><br>
                <form action="edit2.php?action=order" method="POST">
                <div class="text">Nama :</div>
  <input class="inputan" type="text" id="nama" name="nama" value="<?= $d['nama_lengkap']; ?>"><br>
  <div class="text">Pilih Tipe Ruangan :</div>
  <select name="jmeeting" onchange="meeting()" id="jmeeting" class="inputan" >
<option ></option>
<option value="big"> Big </option>
<option value="small"> Small </option>
</select><br>
  <div class="text">Pilih Nomor Ruangan :</div>
  <select id="pilihmeeting" name="idmeeting" class="inputan">
<option value="<?= $d['id_jenis']; ?>"></option>
<option></option>
</select><br>
  <div class="text">Checkin :
   <br></div>
   <input type="date" class="inputan"  id="checkin" name="checkin" value="<?= $d['tgl_checkin']; ?>"><br>

  
  Checkout : <br>
  <input type="date" class="inputan" id="checkout" name="checkout" value="<?= $d['tgl_checkout']; ?>">
  <div class="text">
<br><br>
       <input type="submit" class="btn btn-success" value="EDIT">
       <input type="text" name="jenis" class="hideme">
       <input type="hidden" name="idbooking" value=<?php echo $_GET['id'];?>>
			</div>
		</div>
