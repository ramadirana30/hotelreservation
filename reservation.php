...
<div class="row" >
    <div class="col-xs-12 col-sm-12 col-lg-12" style="background-color:white; height:200px; color : black; font-family : times new roman; font-size : 70px; text-align: center; margin-top : 150px;">MY RESERVATION ROOM AND MEETING<hr style=" border: 2px solid black;"></div>
</div>
<div class="row" >
...
$id_user = $_SESSION['id'];
$reservation = "SELECT id_booking, nama_lengkap, jenis, tipe_jenis,tgl_checkin, tgl_checkout,days,harga,total,kd_booking,status
		FROM tbl_booking WHERE id_user='$id_user'";
$q1 = mysqli_query($koneksi,$reservation);

echo '<table style="width:80%; margin-left:240px; margin-top:-150px;" border="1" class="w3-table-all">
<thead>
<tr class="w3-grey" >
  <th style="color:black;">NO</th>
  <th style="color:black;">NAMA LENGKAP</th>
  <th style="color:black;">JENIS</th>
  <th style="color:black;">TIPE JENIS</th>
  <th style="color:black;">TANGGAL CHECKIN</th>
  <th style="color:black;">TANGGAL CHECKOUT</th>
  <th style="color:black;">DAYS</th>
  <th style="color:black;">HARGA</th>
  <th style="color:black;">TOTAL</th>
  <th style="color:black;">KODE BOOKING</th>
  ...
  <th style="color:black;">AKSI</th>
    ...
    while ($row = mysqli_fetch_array($q1))
    {
      echo '<tr>
          ...
          <td style="color:black;">'.$row['nama_lengkap'].'</td>
          <td style="color:black;">'.$row['jenis'].'</td>
          <td style="color:black;">'.$row['tipe_jenis'].'</td>
          <td style="color:black;">'.$row['tgl_checkin'].'</td>
          <td style="color:black;">'.$row['tgl_checkout'].'</td>
          <td style="color:black;">'.$row['days'].'</td>
          <td style="color:black;">'.$row['harga'].'</td>
          <td style="color:black;">'.$row['total'].'</td>
          <td style="color:black;">'.$row['kd_booking'].'</td>
          <td style="color:black;">'.$row['status'].'</td>
          <td style="color:black;"><button><a style="color:black; text-decoration:none;" href="prosesbatal.php?id='.$row['id_booking'].'">Batalkan Pesanan</a></button></td>
            </tr>';
            $no++;
            if ($row['status'] == 'no')
            {
              ?>
              <script type="text/javascript">$("button").show()</script>
        <?php
            ...
            elseif ($row['status'] == 'yes') {
              ...
              <script type="text/javascript">$("button").hide()</script>
        <?php
            }
